...
<div>${users.email}</div>`
    for (const obj in users) {
        if (typeof (users[obj]) === "object") {
            let address = document.createElement("div");
            let title = document.createElement("h3");
            let geoLocation = document.createElement("div");
            let companyInfo = document.createElement("div");
            for (const element in users[obj]) {
                if (users[obj][element] === users.address[element] && users.address[element] !== users.address.geo) {
                    let addressInfo = document.createElement("div");
                    addressInfo.innerText = `${users.address[element]}`
                    address.append(addressInfo, geoLocation);
                }
                if (users.address[element] === users.address.geo) {
                    for (const elementKey in users.address[element]) {
                        let geo = document.createElement("p");
                        geo.innerText = `${users.address[element][elementKey]}`;
                        geoLocation.appendChild(geo);
                    }
                }
                if (users[obj][element] === users.company[element]) {
                    let company = document.createElement("p");
                    company.innerText = `${users[obj][element]}`
                    companyInfo.appendChild(company);
                }
            }
            userCard.append(title, address, companyInfo);
            title.innerText = `${obj}`;
        }
    }
